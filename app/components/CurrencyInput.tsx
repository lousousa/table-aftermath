import { useEffect, useRef } from "react";

import { getDisplayCurrency } from "@/app/i18n";

type Props = {
  setStateAction: React.Dispatch<React.SetStateAction<string>>;
  maxLength: number;
  autoFocus?: boolean;
  initialValue?: number;
  customClasses?: string;
};

export default function CurrencyInput({
  setStateAction,
  maxLength,
  autoFocus,
  initialValue,
  customClasses,
}: Props) {
  const inputRef = useRef<HTMLInputElement | null>(null);
  const decimalSeparator = getDisplayCurrency() === "USD" ? "." : ",";

  const inputCaretToLastPosition = (
    e: React.SyntheticEvent<HTMLInputElement>,
  ) => {
    if (!(e.target instanceof HTMLInputElement)) return;

    const pos = e.target.value.length;
    e.target.focus();
    e.target.setSelectionRange(pos, pos);
  };

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let value = e.target.value;
    if (!value) return;

    value = value.replace(/\D/g, "");

    if (value.length > maxLength - 1) value = value.substring(0, maxLength - 1);

    while (value.length < 4) {
      value = "0" + value;
    }

    while (value.length >= 4 && value[0] === "0") {
      value = value.substring(1);
    }

    value = `${value.substring(0, value.length - 2)}${decimalSeparator}${value.substring(value.length - 2)}`;

    if (inputRef.current) inputRef.current.value = value;
    setStateAction(parseFloat(value.replace(",", ".")).toFixed(2));
  };

  useEffect(() => {
    if (!inputRef.current) return;

    const value = initialValue
      ? initialValue.toFixed(2).replace(".", decimalSeparator)
      : `0${decimalSeparator}00`;

    inputRef.current.value = value;

    setStateAction(parseFloat(value.replace(",", ".")).toFixed(2));
  }, [inputRef, initialValue, setStateAction, decimalSeparator]);

  return (
    <input
      ref={inputRef}
      type="tel"
      onKeyUp={inputCaretToLastPosition}
      onClick={inputCaretToLastPosition}
      onChange={handleChange}
      autoFocus={autoFocus}
      className={`text-right ${customClasses}`}
    />
  );
}
