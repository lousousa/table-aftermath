...
} from "@/app/store/reducers/payments";
import {
  setStagingItem,
  clearStagingItem,
  removeItemById,
...
export default function InputGrid() {
  ...
  const showResults = () => {
    ...
    itemsList.forEach((item) => {
      const filter = paymentsList.filter(
        (payment) => payment.itemId === item.id && payment.paid,
      );
    ...
    payersList.forEach((payer) => {
      const payerPayments = paymentsList.filter(
        (payment) => payment.payerId === payer.id && payment.paid,
      );
  ...
  const toggleCheckingState = (itemId: number) => {
    const payments = paymentsList.filter(
    ...
    setAllPaid(
      isAllPaymentsPaid ? paidPayments : unpaidPayments,
      !isAllPaymentsPaid,
    );
  };
    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <div className="flex">
          ...
          <div>
            {itemsList.map((item) => (
              <div key={"payer_grid_" + item.id} className="flex items-center">
